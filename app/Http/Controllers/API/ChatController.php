...
class ChatController extends Controller
{
    ...
    public function startPrivateChat($targetUserId)
    {
        $currentUser = auth()->user();
        ...
        if ($currentUser->id === $targetUser->id) {
            return response()->json(['success' => false, 'message' => 'cannot_chat_with_yourself'], 422);
        }

        $existingChat = Chat::where(function($query) use ($currentUser, $targetUser) {
            $query->where('participant_one', $currentUser->id)
                  ->where('participant_two', $targetUser->id);
        })->orWhere(function($query) use ($currentUser, $targetUser) {
            $query->where('participant_one', $targetUser->id)
